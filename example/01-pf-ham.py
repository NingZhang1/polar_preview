...
def get_mol_H2(basis, bond_length=0.746, shift=0.0, dump_input=False):
    mol = gto.Mole(
        atom=[["H", [0, 0, 0.0 - shift]], ["H", [0, 0, bond_length - shift]]],
        basis=basis,
        unit="A",
        verbose=4,
    )
...
def get_mol_HF(basis, bond_length=0.918, shift=0.0, dump_input=False):
    mol = gto.Mole(
        atom=[["H", [0, 0, 0.0 - shift]], ["F", [0, 0, bond_length - shift]]],
        basis=basis,
        unit="A",
        verbose=4,
    )
...
def solve_mol(mol, w, lc, pol_axis, nmode=1, nph=24):
    ...
    mol_fake, H0, hcore, ovlp_lo, eri, w_p, h_ep, rdm1_lo = mole.get_pf_ham(
        mol, w, lc, pol_axis, nmode=nmode
    )
    ...
    return e_elec_hf, e_elec_mp2_1, e_elec_cc_1, e_cshf, e_csmp2, e_csmp2_corr, e_fci
...
if __name__ == "__main__":
    ...
    for sets in settings:
        ...
        e_elec_hf, e_elec_mp2, e_elec_cc, e_cshf, e_csmp2, e_csmp2_corr, e_fci = (
            solve_mol(mol, w, lc, pol_axis, nmode=nmode, nph=24)
        )
